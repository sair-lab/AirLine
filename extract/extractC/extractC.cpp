				...
				while (searchEnd == 0) {
					...
					if (candidateNum[!pingpong] == 0) {//no more candidate
						...
						if (count > leastNumofPx && count < 5000) {
							...
							o(lineNum, 0, 0) = centerOfMass[0] - vy * pmaxLength;
							o(lineNum, 0, 1) = centerOfMass[1] - vx * pmaxLength;
							o(lineNum, 1, 0) = centerOfMass[0] + vy * nmaxLength;
							o(lineNum, 1, 1) = centerOfMass[1] + vx * nmaxLength;
							lineNum++;
							//cout << "search end with count" << count << endl;
						}
					}

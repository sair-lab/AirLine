...
if __name__ == '__main__':
    ...
    for o in os.listdir(path):
        ...
        dscale=1
        if demo:
            #rx1=cv2.resize(rx1,(rx1.shape[1]//2,rx1.shape[0]//2))
            pass
        if not usingUnet:
            rx1=cv2.resize(rx1,(rx1.shape[1]//dscale//res*res,rx1.shape[0]//dscale//res*res))
        #rx1=cv2.resize(rx1,(rx1.shape[1]//4,rx1.shape[0]//4))
        ...
        THETADes=thetaN(lineDetection1)
        imggradient=torch.cat([x1.unsqueeze(0)-x1.roll(1,1).unsqueeze(0),x1.unsqueeze(0)-x1.roll(0,1).unsqueeze(0)])
        #THETAPrediction=thetaN(torch.tensor(lineDetection1).unsqueeze(0).unsqueeze(0).cuda().float()/255)
        THETADes=torch.nn.functional.normalize(THETADes-THETADes.mean(1), p=2.0, dim=1)
        #THETADes=torch.nn.functional.normalize(imggradient.unsqueeze(0), p=2.0, dim=1)
        #THETADes=torch.nn.Softmax()(THETADes)
        ...
        #print("start")
        #edgeNp=(edgeNp>-2.3).astype(np.uint8)*255#1.5
        ...
        #linenum=EC.desGrow(outMap,(edgeNp>0),THETADes[0].detach().cpu().numpy(),out,0.99,30,tempMem,tempMem2,tempMem3,THETARESOLUTION)#lineDetection1.detach().cpu().numpy()[0,0]
        if not demo:
